...
func getSeeds(arr []int) []int {
	seeds := make([]int, 0)
	for i := 0; i < len(arr); i += 2 {
		for j := 0; j < arr[i+1]; j++ {
			seeds = append(seeds, arr[i]+j)
		}
	}
	return seeds
}
...
func buidAlmanac(lines []string) Almanac {
	...
	rangedSeeds := getSeeds(seeds)

	almanac := Almanac{seeds: rangedSeeds}
...
// Takes a while to run, was too lazy to find a more efficient method
func main() {
